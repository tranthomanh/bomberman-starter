...
public class Minvo extends Entity{
    ...
    public void update(BombermanGame gp) {
        ...
        if(isLive == false){
            if(countDead > 0){
                setImage(Sprite.minvo_dead.getFxImage());
                countDead --;
            }
            else return;
        }
    ...
    public void render(GraphicsContext gc) {
        ...
        counter++;
        if(type == 0){
            Sprite sprite = Sprite.movingSprite(Sprite.minvo_left1, Sprite.minvo_left2, Sprite.minvo_left3, counter , 30);
            gc.drawImage(sprite.getFxImage(), x, y);
        }
        else{
            Sprite sprite = Sprite.movingSprite(Sprite.minvo_right1, Sprite.minvo_right2, Sprite.minvo_right3, counter , 30);
            gc.drawImage(sprite.getFxImage(), x, y);
        }
    }
}
